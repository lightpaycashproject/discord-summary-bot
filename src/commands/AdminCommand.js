...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('clear-cache')
        .setDescription('Clears the summary and X scraping cache for the current channel')
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('stats')
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'clear-cache') {
      try {
        ...
        await interaction.reply({ content: `✅ Cache cleared for channel <#${interaction.channelId}>.`, ephemeral: true });
      } catch (error) {
        console.error('Admin clear-cache error:', error);
        await interaction.reply({ content: 'Failed to clear cache.', ephemeral: true });
      }
